...
- **Windows** (`profile.ps1`) — passed via `pwsh -NoLogo -NoExit -ExecutionPolicy Bypass -File <path>`. Wraps the user's existing `prompt` function (after their `$PROFILE` runs) to emit OSC 7 + OSC 133 A/B/D. Shell priority: `pwsh.exe` (PS 7+) → `powershell.exe` (PS 5.1) → `cmd.exe` (no integration). cwd is normalized to backslashes before being passed to ConPTY (`CreateProcessW` misbehaves with forward-slash cwd). Profile.ps1 writes a no-op cursor-visible sequence at the top to prime the ConPTY output pipe — without it, the first prompt sometimes stalls until input forces a flush.

`pty/shell_init.rs` is split into `#[cfg(unix)]` / `#[cfg(windows)]` modules — keep new platform-specific code in the right cfg arm.

ConPTY on Windows requires `SPAWN_LOCK` (Mutex) around `openpty + spawn_command` in `session.rs` — concurrent spawns from React 18 strict-mode double-mount otherwise leave one or both PTYs connected but stalled. Don't remove the lock without verifying first-tab stability on Windows.

`session.rs` also runs a Windows-only `terax-pty-conpty-nudge` watchdog. Cold pwsh starts (and bursts of parallel spawns) leave the master->host pipe stalled — child is alive, reader is parked on `ReadFile`, prompt never reaches xterm. The watchdog calls `master.resize(size)` with the same dimensions at 400/900/1600ms if `bytes_received == 0`; ConPTY's resize path goes through a separate control channel and reliably forces the agent to flush. The `profile.ps1` prime-write covers the secondary stall *after* pwsh reaches the profile; the watchdog covers the primary stall *before* pwsh emits anything. Keep both — they handle disjoint failure modes.
