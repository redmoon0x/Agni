...
if [ -z "$__AGNI_HOOKS_LOADED" ]; then
  ...
  _agni_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_agni_urlencode "$PWD")"
    if [ -z "$__AGNI_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __AGNI_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_agni_precmd:"*) ;;
    *) PROMPT_COMMAND="_agni_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
