...
# zsh, so we shadow $? into `_agni_ret`.

{
  _agni_user_zdotdir="${AGNI_USER_ZDOTDIR:-$HOME}"
  [ -f "$_agni_user_zdotdir/.zshrc" ] && source "$_agni_user_zdotdir/.zshrc"
...
if [[ -z "$__AGNI_HOOKS_LOADED" ]]; then
  __AGNI_HOOKS_LOADED=1
  ...
  _agni_precmd() {
    local _agni_ret=$?
    printf '\e]133;D;%s\e\\' "$_agni_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_agni_urlencode "$PWD")"
  ...
  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _agni_precmd
    add-zsh-hook preexec _agni_preexec
  fi
