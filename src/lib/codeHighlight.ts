// Lezer-based syntax highlighting for markdown code blocks.
// Originally from ai-elements/chat-code-lezer.ts — kept because it has zero AI deps.
...
const streamLoaders: Record<string, StreamLoader> = {
  c: () => import("@codemirror/legacy-modes/mode/clike").then((m) => m.c as unknown as StreamParser<unknown>),
  cpp: () => import("@codemirror/legacy-modes/mode/clike").then((m) => m.cpp as unknown as StreamParser<unknown>),
  java: () => import("@codemirror/legacy-modes/mode/clike").then((m) => m.java as unknown as StreamParser<unknown>),
  csharp: () => import("@codemirror/legacy-modes/mode/clike").then((m) => m.csharp as unknown as StreamParser<unknown>),
  kotlin: () => import("@codemirror/legacy-modes/mode/clike").then((m) => m.kotlin as unknown as StreamParser<unknown>),
  scala: () => import("@codemirror/legacy-modes/mode/clike").then((m) => m.scala as unknown as StreamParser<unknown>),
  objectivec: () => import("@codemirror/legacy-modes/mode/clike").then((m) => m.objectiveC as unknown as StreamParser<unknown>),
  dart: () => import("@codemirror/legacy-modes/mode/clike").then((m) => m.dart as unknown as StreamParser<unknown>),
  yaml: () => import("@codemirror/legacy-modes/mode/yaml").then((m) => m.yaml as unknown as StreamParser<unknown>),
  toml: () => import("@codemirror/legacy-modes/mode/toml").then((m) => m.toml as unknown as StreamParser<unknown>),
  ruby: () => import("@codemirror/legacy-modes/mode/ruby").then((m) => m.ruby as unknown as StreamParser<unknown>),
  swift: () => import("@codemirror/legacy-modes/mode/swift").then((m) => m.swift as unknown as StreamParser<unknown>),
  lua: () => import("@codemirror/legacy-modes/mode/lua").then((m) => m.lua as unknown as StreamParser<unknown>),
  haskell: () => import("@codemirror/legacy-modes/mode/haskell").then((m) => m.haskell as unknown as StreamParser<unknown>),
  perl: () => import("@codemirror/legacy-modes/mode/perl").then((m) => m.perl as unknown as StreamParser<unknown>),
  r: () => import("@codemirror/legacy-modes/mode/r").then((m) => m.r as unknown as StreamParser<unknown>),
  dockerfile: () => import("@codemirror/legacy-modes/mode/dockerfile").then((m) => m.dockerFile as unknown as StreamParser<unknown>),
  nginx: () => import("@codemirror/legacy-modes/mode/nginx").then((m) => m.nginx as unknown as StreamParser<unknown>),
  diff: () => import("@codemirror/legacy-modes/mode/diff").then((m) => m.diff as unknown as StreamParser<unknown>),
};
...
function highlightStream(code: string, parser: StreamParser<unknown>): HighlightedNode[] {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    while (!stream.eol()) {
      ...
      if (!text) continue;
      out.push({ kind: "text", value: text, cls: tag ? mapStreamTag(tag) : "" });
    }
  }
  return out;
}
...
function mapStreamTag(raw: string): string {
  return raw
    ...
    .map((t) => {
      ...
      switch (base) {
        case "variable": return "tok-variableName";
        case "variable-2": return "tok-variableName";
        case "def": return "tok-definition tok-variableName";
        case "property": return "tok-propertyName";
        case "type": return "tok-typeName";
        case "builtin": return "tok-name";
        case "atom": return "tok-atom";
        case "tag": return "tok-tagName";
        case "attribute": return "tok-attributeName";
        case "meta": return "tok-meta";
        case "qualifier": return "tok-modifier";
        case "operator": return "tok-operator";
        case "bracket": return "tok-bracket";
        case "punctuation": return "tok-punctuation";
        case "header": return "tok-heading";
        case "link": return "tok-link";
        case "string": return "tok-string";
        case "string-2": return "tok-string";
        case "comment": return "tok-comment";
        case "number": return "tok-number";
        case "keyword": return "tok-keyword";
        default: return `tok-${base}`;
      }
    })
    .join(" ");
}
...
export async function highlight(
  ...
  if (r.kind === "lezer") {
    ...
    const out: HighlightedNode[] = [];
    highlightCode(code, tree, classHighlighter, (text, cls) => {
