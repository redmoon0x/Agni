...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-agni-recycler", "");
  el.style.cssText =
...
function createSlot(): Slot {
  ...
  host.setAttribute("data-agni-slot", String(slots.length));
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[agni] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function attachWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[agni-webgl] unavailable:", e);
  }
}
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[agni-webgl] dispose failed:", e);
  }
  try {
    const r = (
